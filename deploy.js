...
function transferProjectToRemote(failed, successful) {
  return ssh.putDirectory(
    '../hands-on-three',
    '/home/ubuntu/hands-on-three-temp',
    {
      recursive: true,
      concurrency: 1,
...
function createRemoteTempFolder() {
  return ssh.execCommand(
    'rm -rf hands-on-three-temp && mkdir hands-on-three-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function updateRemoteApp() {
  return ssh.execCommand(
    'mkdir hands-on-three && cp -r hands-on-three-temp/* hands-on-three/ && rm -rf hands-on-three-temp', {
      cwd: '/home/ubuntu'
  });
}
...
function restartRemoteServices() {
  return ssh.execCommand(
    'cd hands-on-three && sudo service mongod start && pm2 start app.js', {
      cwd: '/home/ubuntu'
  });
}
...
function sshConnect() {
  ...
  ssh
    ...
    .then(function() {
      console.log('Creating `hands-on-three-temp` folder.');
